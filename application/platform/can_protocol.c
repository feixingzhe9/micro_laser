...
#define  SENSOR_LIGHT_NUM      (13)
#define  SENSOR_UTRLA_NUM      (9)
#define  SENSOR_NUM            (SENSOR_LIGHT_NUM+SENSOR_UTRLA_NUM)
uint8_t SensorData[SENSOR_NUM];
uint8_t LightData;
...
void SysSensorMsgReq(uint8_t type, uint8_t index)
{
    uint8_t i, sensor_base;
    CAN_ID_UNION id;
    CAN_DATA_UNION data;
    ...
    id.CanID_Struct.SrcMACID = 0x60;
    id.CanID_Struct.DestMACID = sensor_base + index;
    id.CanID_Struct.ACK = 0;
    id.CanID_Struct.FUNC_ID = 2;
    id.CanID_Struct.SourceID = 0x80;
    
    data.CanData_Struct.SegPolo = 0;
    data.CanData_Struct.SegNum = 0;
    
    for(i = 0; i < 7; i++)
    {
        data.CanData_Struct.Data[i] = 0;
    }
    
    CanTX(MICO_CAN1, id.CANx_ID, &data.CanData[1], 3);
}
...
uint16_t CmdProcessing(CAN_ID_UNION *id, const uint8_t *data_in, const uint16_t data_in_len, uint8_t *data_out)
{
    ...
    tmp_id = id->CanID_Struct.DestMACID; 
    id->CanID_Struct.SrcMACID = tmp_id;
        ...
        case CAN_FUN_ID_READ:
            switch(id->CanID_Struct.SourceID)
            {
                ...
                case CAN_SOURCE_ID_READ_MEASURE_DATA:
                     data_out[1] = LightData;
                     return (2);
                    break;
                    
                case CAN_SOURCE_ID_DATA_TO_ROS: 
                     memcpy(&data_out[1], SensorData,sizeof(SENSOR_NUM));         
                     return (SENSOR_NUM);
                    break;
...
void can_light_send(void)
{
      ...
      light_id.CanID_Struct.SourceID = 0x80;
    ...
    {  
        ...
        if(seg_polo == ONLYONCE)
        {
            ...
            if(id.CanID_Struct.SrcMACID == 0x60)
            {
                  ...
                  rxdata[0] = real_data;
                  light_id.CanID_Struct.SrcMACID  = id.CanID_Struct.DestMACID;
                  light_id.CanID_Struct.DestMACID = id.CanID_Struct.SrcMACID;
                  light_id.CanID_Struct.ACK = 1;
                  light_id.CanID_Struct.FUNC_ID = id.CanID_Struct.FUNC_ID;
                  light_id.CanID_Struct.SourceID = id.CanID_Struct.SourceID;
                  light_id.CanID_Struct.res = 0;
                  CanTX( MICO_CAN1, light_id.CANx_ID, rxdata, 2);
            }
